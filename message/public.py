...
class OpenConfig(object):
    ...
    """
    def __init__(self):
        self.path = os.path.join(BASE_DIR, "mseeage/tempates/config.xml")
        self.file = open(self.path, "r")
        self.lines = self.file.readlines()
        self.file.close()
        self.xml_tree = ET.fromstring(self.lines)

    def getAppId(self):
        self.appID = self.xml_tree.find("appID")
        return self.appID

    def getAppSecret(self):
        self.appSecret = self.xml_tree.find("appSecret")
